...
export class TwitchController {
  ...
  async checkNasaLive(@Headers('authorization') authorization: string, @Response() reply: FastifyReply) {
    try {
      const nasaTwitchId = "151920918"
      const response = await this.twitchService.checkNasaLive(nasaTwitchId, authorization)
